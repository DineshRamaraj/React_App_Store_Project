...
class AppStore extends Component {
  state = {
    selectedItem: tabsList[0].tabId,
    searchInput: '',
  }

  onClickRandomTabButton = id => {
  ...
  render() {
    const {selectedItem, searchInput} = this.state
    const filteredItem = appsList.filter(
      eachItem => eachItem.category === selectedItem,
    )
    const searchResults = filteredItem.filter(eachItem =>
      eachItem.appName.toLowerCase().includes(searchInput.toLowerCase()),
    )
    // const filteredAndSearchItem =
    //   searchInput.length > 0 ? searchResults : filteredItem
        ...
        <ul className="tab-list-container">
          {tabsList.map(each => (
            <TabItem
              key={each.tabId}
              eachItem={each}
              isActive={selectedItem === each.tabId}
              onClickRandomTabButton={this.onClickRandomTabButton}
            />
          ))}
        </ul>
